...
let configureCommand = "git clone https://github.com/aishwaryapradeep01/CPDemoFramework.git -b ${BRANCH} --single-branch ${CHE_PROJECTS_ROOT}/techzone-demo;cd ${CHE_PROJECTS_ROOT}/techzone-demo/olm-utils-v2;sh configure-env.sh ";
...
function install_cpd() {
    ...
    let storage = "auto"
    document.getElementById("install_cpd$1").setAttribute("command", "cd ${CHE_PROJECTS_ROOT}" + `/techzone-demo/olm-utils-v2/;pip3.8 install PyYAML;python3.8 updateYaml.py  ${component_list} ${storage} ${cp4waiopsVersion} cp4waiops ${instana_sales_key} ${instana_agent_key} ${turbonomic_lic} ; bash deploy.sh cp4waiops ${cp4waiopsAdminPassword} ${cp4waiopsEnvName}`)
